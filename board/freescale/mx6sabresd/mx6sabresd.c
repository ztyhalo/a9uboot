...
#define PM_3V6_PWREN	IMX_GPIO_NR(1, 13)	//GPIO1_IO13
#define PM_3V3_A_PWREN	IMX_GPIO_NR(6, 15)	//GPIO6_IO15
#define PM_1V8_PWREN	IMX_GPIO_NR(1, 14)	//GPIO1_IO14
#define PM_1V2_PWREN	IMX_GPIO_NR(1, 15)	//GPIO1_IO15
#define PM_3V3_B_PWREN	IMX_GPIO_NR(7, 12)	//GPIO7_IO12
#define PM_GPS_PWREN	IMX_GPIO_NR(1, 4)	//GPIO1_IO04


#define PM_ENABLE	1

iomux_v3_cfg_t const pm_pads[] = {
...
static void setup_iomux_pm(void)
{
	puts("Main&IO Board Power Enable!\n");
	imx_iomux_v3_setup_multiple_pads(pm_pads, ARRAY_SIZE(pm_pads));
	gpio_direction_output(PM_3V6_PWREN, PM_ENABLE);
	gpio_direction_output(PM_3V3_A_PWREN, PM_ENABLE);
	gpio_direction_output(PM_1V8_PWREN, PM_ENABLE);
	gpio_direction_output(PM_1V2_PWREN, PM_ENABLE);
	gpio_direction_output(PM_3V3_B_PWREN, PM_ENABLE);
	gpio_direction_output(PM_GPS_PWREN, !PM_ENABLE); //low
}
#endif
...
#define GPIO_DR			0x0
#define GPIO_GDIR		0x4	/* direction register */
#define GPIOn_BASE_ADDR		GPIO7_BASE_ADDR         //0x020B4000
#define GPIO_BITn_MASK		(1<<8)

void hw_watchdog_pin_init(void)
{
	u32 reg;
	imx_iomux_v3_setup_pad(MX6_PAD_SD3_RST__GPIO7_IO08 | MUX_PAD_CTRL(NO_PAD_CTRL));
	reg = readl(GPIOn_BASE_ADDR + GPIO_GDIR);
	/* set watchdog gpio as output */
	reg |= GPIO_BITn_MASK;
	writel(reg, GPIOn_BASE_ADDR + GPIO_GDIR);
}

void hw_watchdog_reset(void)
{
	u32 reg;
	//printf("GPIOn_BASE_ADDR=%x\n", GPIOn_BASE_ADDR);
	/*  watchdog change between 0 and 1 */
	reg = readl(GPIOn_BASE_ADDR + GPIO_DR);
	reg ^= GPIO_BITn_MASK;
	writel(reg, GPIOn_BASE_ADDR + GPIO_DR);
}
#endif
...
static void setup_iomux_enet(void)
{
	...
	gpio_set_value(IMX_GPIO_NR(3, 18), 0);
...
int mx6_rgmii_rework(struct phy_device *phydev)
{
	...
	#define BC_REG	0x0
	val = phy_read(phydev, MDIO_DEVAD_NONE, BC_REG);
	val |= 1<<15;
	phy_write(phydev, MDIO_DEVAD_NONE, BC_REG, val);
...
void read_mac_ds2460()
{
	int i, ret;
	char *ethprime = NULL;
	char strbuf[50] = { 0 };
	unsigned char buf[6] = { 0 };
	struct eth_device *ethdevice = NULL;
	ret = ds2460_read(0x80, buf, 6);
	if (ret != 6) {
		printf("ds2460 read fail, return %d\n", ret);
		return ;
	}
	/*
	ethprime = getenv ("ethprime");
	ethdevice = eth_get_dev_by_name(ethprime);
	if (ethprime == NULL || ethdevice == NULL) {
		printf("net init not compelet: ethedevice:%x, ethprime %s\n", ethdevice, ethprime);
		return ;
	}
	memcpy(ethdevice->enetaddr, buf, sizeof(buf));
	printf("Read MAC from ds2460 : %pM\n", ethdevice->enetaddr);
	sprintf(strbuf, "%pM", ethdevice->enetaddr);
	*/
	printf("Read MAC from ds2460 : %pM\n", buf);
	sprintf(strbuf, "%pM", buf);
	//setenv("fec_mac", strbuf);
	setenv("ethaddr", strbuf);
...
void dcp5k_usb_hub_reset()
{
	...
	imx_iomux_v3_setup_pad(hub_reset_pads);
	puts("Reset USB HUB!\n");
...
int isanywheremode()
{
	...
	ds2460_read(0x80, buf, 8);
	if ((buf[0] == 0x00) && (buf[1] == 0x14) && (buf[2] == 0x97))
		return 0;
	else
		return 1;
}
