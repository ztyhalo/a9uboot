...
{
	u16 reg;
	printf("www rtl8211f_config\n");
	phy_write(phydev, MDIO_DEVAD_NONE, MII_BMCR, BMCR_RESET);
	mdelay(200);

	//1000m led green
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_RTL8211F_PAGE_SELECT, 0xd04);
	phy_write(phydev, MDIO_DEVAD_NONE, 0x10, 0x6359);
	...
	reg = phy_read(phydev, MDIO_DEVAD_NONE,0x11);
	printf("ydx page 0xd08 register 0x11 is 0x%x\n",reg);

	//rx delay enable
	phy_write(phydev, MDIO_DEVAD_NONE, 0x15, 0x19);
	reg = phy_read(phydev, MDIO_DEVAD_NONE,0x15);
	printf("ydx page 0xd08 register 0x15 is 0x%x\n",reg);
